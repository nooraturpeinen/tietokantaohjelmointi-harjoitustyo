...
function allPosts() {
    ...
    try {
        $db = openDb();
        $sql = 'select post.id as id, user.username as username, title, post, date_format(created, "%d.%m.%Y %H:%i") as created, picture from post inner join user on post.user_id = user.id order by post.id desc';
        $posts = $db->query($sql);
...
function allComments($post_id) {
    ...
    try {
        $db = openDb();
        $sql = 'select comment.id as id, post_id, user.username as username, comment, date_format(comment.created, "%d.%m.%Y %H:%i") as created, picture from comment inner join user on comment.user_id = user.id where post_id = ?';
...
function allUsersPosts() {
    ...
    try {
        $db = openDb();
        $sql = 'select post.id as id, user.username as username, title, post, date_format(created, "%d.%m.%Y %H:%i") as created, picture from post inner join user on post.user_id = user.id where user_id = ? order by post.id desc';
